...
import { cmdBuyPrice, cmdSellPrice } from "../lib/cli-get-info";
...
describe("cmdBuyPrice()", (): void => {
	...
	it("should output the price", (): void => {
		...
		expect(output.logs).to.deep.equal(["1 BTC = 15235.00 EUR"]);
	});
});
...
describe("cmdSellPrice()", (): void => {
	...
	it("should output the price", (): void => {
		...
		expect(output.errors.length).to.equal(0);
		expect(output.logs).to.deep.equal(["1 BTC = 15234.00 EUR"]);
	});
});
